...
Muro offers full support for exporting Netflow data to external collectors as
well as a comprehensive Analyzer for on-the-box analysis and live monitoring.

Muro is the only open source solution with a built-in Netflow analyzer integrated
into its Graphical User Interface. It can be accessed via :menuselection:`Reporting --> Netflow`.

------------------
Supported Versions
------------------
Muro support both Netflow version 5 (IPv4) and version 9 (IPv4 & IPv6).
...
If you are not interested in ingress traffic then Muro offers the option to
...
Netflow Exporter
----------------
Muro Netflow Exporter supports multiple interfaces, filtering of ingress flows
and multiple destinations including local capture for analysis by Insight (Muro
Netflow Analyzer).
...
--------------------------
Muro offers a full Netflow Analyzer with the following features:
