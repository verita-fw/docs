...
Muro supports VPN connections for branch offices as well as remote users.
...
Muro offers a wide range of VPN technologies ranging from modern SSL VPNs to
...
The following functions are available in the menu (as of Muro 23.1):
...
IPsec may also be used to service remote workers connecting to Muro from various clients, such as Windows, MacOS, iOS and Android.
...
The examples section contains various options available in Muro. When using the new "connections" option available
as of Muro 23.1, different `examples from Strongswan <https://docs.strongswan.org/docs/5.9/interop/windowsClients.html>`__
...
This new component will eventually replace the existing client and server options in a future version of Muro, leaving
enough time to migrate older setups.

.. Tip::

  When upgrading into a new major version of Muro, always make sure to read the release notes to check if your setup
  requires changes.

.. Note::

  OpenVPN on Muro can also be used to create a tunnel between two locations, similar to what IPsec offers. Generally
...
The following functions are available in the menu (as of Muro 23.7):
...
Muro are  :doc:`grouped <firewall_groups>` as `OpenVPN`.
