...
The user manager of Muro allows for controlling access to the different
part (pages) of the configurator as well as controlling access to particular
services on a per user bases.

Authentication
--------------
Muro offers integration with external servers for services that require user
...
Integrated into Muro are the **Local User Database** and **Voucher Server**.
...
privileges to the users they need to be imported in Muro's local user manager.
...
By default, Muro GUI login will use local accounts. This can be changed, however,
