...
.. Note::
  The scope of Wazuh on Muro is only to offer configurable agent support. We do not plan nor advise to run the Wazuh
  central components on Muro. Detailed information on how to install these on supported platforms are available directly from the
...
.. Warning::
  This plugin is provided "as-is" and with very limited [tier 3] community support from the Muro team. Using a SIEM/XDR system
...
For Intrusion detection we can send the events as well using the same (eve) datafeed used in Muro, just mark the
...
You can add these in :code:`/usr/local/opnsense/service/templates/Muro/WazuhAgent/ossec_config.d/`, for example, to
add a custom json feed, add a file containing the following content in there:

.. code-block:: xml
  :linenos:
  :caption: /usr/local/opnsense/service/templates/Muro/WazuhAgent/ossec_config.d/099-my-feed.conf
