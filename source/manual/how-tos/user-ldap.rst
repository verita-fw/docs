...
Muro can use an LDAP server for authentication purposes and for authorization
...
Your Muro firewall needs to be fully configured and able to access the LDAP server.
...
.. Note::
    ...
    Groups will be extracted from the first :code:`CN=` section and will only be considered when already existing in Muro.
...
.. Note::
    ...
    (not available in the community version of Muro)
