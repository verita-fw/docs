...
One of the more powerful features of Muro is to set-up a redundant
...
Muro includes a mechanism to keep the configuration of the backup
...
default NAT configuration is for Muro is to use Automatic outbound NAT 
...
each others pools. If DNS requests are also forwarded by Muro, make
...
First go to :menuselection:`System --> High availability --> Status` in the Muro webinterface and check if
...
connected you should be able to look at the state table on both Muro
