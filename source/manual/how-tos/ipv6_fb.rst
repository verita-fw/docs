...
The Muro has an interface pointing to the ISP named `WAN` and has three internal 
...
the Muro host. Select :menuselection:`Internet --> Permit Access --> <your OPN Host> --> IPv6 Settings --> Open firewall for delegated IPv6 prefixes of this device`
...
2. the setting **Request only an IPv6 prefix** is the important part. 
   ...
   the Muro as a router and really delegates a prefix. The OPNSense will only get a link-local `0xfe80`
   address but that is fine. If this checkbox is not selected the FB considers the Muro as an end-user device
   and plainly refuses to delegate a prefix to it. The Muro end up with an valid IPv6 address but with `/64`
...
DNS options                  empty        this gives away the Muro as DNS server with the current dynamic IP
===========================  ===========  ======================================================================
...
While discovering the specifics of IPv6 behind a FB in combination with Muro the first point of debugging was always
connecting via SSH to Muro on the CLI. 
