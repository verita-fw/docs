...
The Muro configuration can be stored as a backup file in XML format,
...
unlimited traffic, a remote backup of an Muro configuration file is
...
Remote backup
-------------
In Muro\ :sup:`1` you can **backup** your configuration directly and
...
After set-up, the backup feature will run a first backup of the Muro
...
-  On the next page enter a name for your project, for example "Muro Backups". You may leave it as the default name ("My Project #####"), as it does not really matter. Then click on the **CREATE** button:
...
-  On the next page, make sure your project name (for example "Muro Backups") is displayed at the top of the page - if not, select it in the dropdown. Then click the **ENABLE** button:
...
-  Enter an account name (for example "Muro") and a description (for example "Muro Backups service account"), then click the **CREATE** button:
...
-  Enter a name for the folder (for example "Muro Backups", it doesn't really matter) and then click the **CREATE** button:
...
3. Set up the account in Muro
=================================

Now put it all together in Muro.

-  Log into your Muro firewall and go to the backup feature. It is located at :menuselection:`System --> Configuration --> Backups`.
-  Scroll down to the Google Drive section and and enter the following values:
    ...
    Prefix hostname to backupfile    checked or unchecked as desired (useful if you have multiple Muros)
...
-  Then click the **Setup/Test Google Drive** button. Muro will automatically save and test your settings and you will receive either an error (connectivity issues) or a message saying "Backup successful" with a list of files currently in the backup.
...
3. Step Connect Muro with Nextcloud
=======================================
...
:sup:`1` As of Muro version 15.1.8.2 (25 March 2015)
