...
Besides the configuration options that every component has, Muro also contains a lot of general settings
...
The settings on this page concerns logging into Muro. The “Secure Shell” settings are described under
...
DNS Rebind Check                               Muro contains protection against
...
In order to access Muro via SSH, SSH access will need to be configured via :menuselection:`System --> Settings --> Administration`.
...
============================================== ========================================================================
Use the virtual terminal driver (vt)           When unchecked, Muro will use the older sc driver.                |
...
+---------------------------------+------------------------------------------------------------------------------------+
| Do not use the local DNS        | When enabling local DNS services such as Dnsmasq and Unbound, Muro will use    |
...
A list of possible values can be obtained by issuing ``sysctl -a`` on an Muro shell.
...
As of Muro 20.7 we changed our default logging method to regular files.
...
With Muro version 19.7, syslog-ng for remote logging was introduced.
...
.. Note::
    ...
    errors are quite common in these type of setups. On Muro the general system log usually contains more details.
