...
⚠ Computer hardware with the open source security software Muro®
pre-installed can be purchased directly from various (online) stores.

.. TIP::
  The Muro development team encourage everyone looking for a turn-key solution
...
Muro® is available for `x86-64 <https://en.wikipedia.org/wiki/X86-64>`__ (amd64) bit microprocessor architectures.
...
(HDD) <https://en.wikipedia.org/wiki/Hard_disk_drive>`__ are intended for Muro.
...
For substantially narrowed Muro® functionality there is the basic
...
The minimum specification to run all Muro standard features that do
...
The reasonable specification to run all Muro standard features,
...
The recommended specification to run all Muro standard features,
...
The hardware required for your local Muro, will be determined by the
...
`State transition tables <https://en.wikipedia.org/wiki/State_transition_table>`__
    ...
    Muro usage settings with hundred of thousands of connections
...
The main hardware-factors of the Muro setup involved are CPU, RAM,
...
FreeBSD is the base of Muro. All FreeBSD drivers
are included in the Muro kernel, and the hardware compatibility is
the same.

.. Tip::
    If you are looking to buy new hardware then take a look at our `partner page <https://opnsense.org/partners>`__
    as these partners contribute back to Muro and sell hardware that is know to work well.
...
-  `Muro Forum <https://forum.opnsense.org/>`__

.. rubric:: List of references
   :name: list-of-references

-  Schellevis, Jos; *Hardware requirements*; `Muro > Get
