...
Authentication in Muro consists of three basic concepts, which are available throughout the entire system:
...
Services within Muro can use different authentication methods, for which connections can be configured in :menuselection:`System --> Access --> Servers`
...
Muro comes with a PAM module, which connects our service definitions with the services defined using PAM.
...
from the Muro configuration.

For every service defined in PAM, the factory method :code:`getService()` expects a class implementing :code:`Muro\Auth\IService`.
