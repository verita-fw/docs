...
The firewall API plugin (**os-firewall**) offers a way for machine to machine interaction between custom applications and Muro, it can
...
.. csv-table:: Abstract [non-callable] (FilterBaseController.php) 
   ...
   :widths: 4, 15, 15, 30, 40
    ...
    "``<<uses>>``", "", "", "", "*model* `Filter.xml <https://github.com/opnsense/plugins/blob/master/net/firewall/src/opnsense/mvc/app/models/Muro/Firewall/Filter.xml>`__"
...
The firewall plugin injects rules in the standard Muro firewall while maintaining visibility on them in the
...
a rule named "Muro_fw_api_testrule_1", when not found one will be added otherwise it will print the internal uuid.
