...
While the latter will be of lesser importance for Muro, since it specifically applies to connections
...
When Suricata is running in IPS mode, Netmap is utilized to fetch packets off the line for inspection. By default, Muro has configured Suricata in such a way that the packet which
