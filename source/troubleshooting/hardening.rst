...
Muro tends to choose more strict hardening options by default, so when comparing performance between upstream
...
In Muro IBRS is enabled (for Intel) by default by disabling (0) :code:`hw.ibrs_disable`, upstream FreeBSD standard is disabled (1).
